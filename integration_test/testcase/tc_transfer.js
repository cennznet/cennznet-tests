...
describe('Transfer Token test suite:', function () {
    ...
    it('Transfer staking token', async function() {
        ...
        // get bal before tx
        const beforeTx_cennz = await queryFreeBalance(toAddress, CURRENCY.STAKE)
        const beforeTx_spend = await queryFreeBalance(toAddress, CURRENCY.SPEND)

        // transfer
        await transfer(fromSeed, toAddress, transAmt, assetId)

        // get bal after tx
        const afterTx_cennz = await queryFreeBalance(toAddress, CURRENCY.STAKE)
        const afterTx_spend = await queryFreeBalance(toAddress, CURRENCY.SPEND)
